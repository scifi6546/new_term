...
impl TextRender {
    ...
    pub fn update(&mut self, key: Option<winit::event::VirtualKeyCode>) {
        if let Some(key) = key {
            self.buffer.push(VKeyCode_to_char(key));
            println!("key: {}", VKeyCode_to_char(key));
        }
    }
}
fn VKeyCode_to_char(key: winit::event::VirtualKeyCode) -> char {
...
impl crate::Updater for TextRender {
    fn update(&mut self, image: &mut image::RgbaImage) {
        ...
        *image = image::RgbImage::from_raw(x, y, self.canvas.pixels.clone()).unwrap().convert();
    }
}
